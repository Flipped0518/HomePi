...
public class ChooseAreaActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		homePiDB = HomePiDB.getInstance(this);
	...
	private void queryProvinces(){
		provinceList = homePiDB.loadProvinces();
	...
	private void queryCities() {
		cityList = homePiDB.loadCities(selectedProvince.getId());
		if (cityList.size() > 0) {
			dataList.clear();
	...
	/*
	 * 查询选中市内所有的县，优先从数据库查询，如果没做再去服务器上查询
	 */
	private void queryCounties(){
		countyList = homePiDB.loadCounties(selectedCity.getId());
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener(){
			...
			public void onFinish(String response) {
				...
				if ("province".equals(type)) {
					result = Utility.handleProvincesResponse(homePiDB, response);
				}else if ("city".equals(type)) {
					result = Utility.handleCitiesResponse(homePiDB, response, 
							selectedProvince.getId());
				}else if ("county".equals(type)) {
					result = Utility.handleCountiesResponse(homePiDB, response, 
							selectedCity.getId());
				}
				if (result) {
	...
	/*
	 * 捕获Back按键，当前的级别来判断，此时应该返回市列表、省列表还是直接退出。
	 */
	@Override
